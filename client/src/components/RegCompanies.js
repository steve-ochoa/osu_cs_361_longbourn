...
export default function RegCompanies(props) {
  ...
  async function handleSubmit() {
    ...
    if (updatedFields.length === 1 && location.pathname === "/register4") {
      if (
        updatedFields[0].name === "" &&
        updatedFields[0].description === "" &&
        updatedFields[0].industry === "" &&
        updatedFields[0].position === "" &&
        updatedFields[0].years === ""
      ) {
        history.push({
  ...
  return (
    <>
      <div className={"registerCompaniesHeader"}>
        <h2>Add Work Experience</h2>
        <br />
        <p>
          Which companies have you worked at?
          <br />
          Add a company by entering the company name (e.g. "NVIDIA", or
          "Microsoft") and selecting it from the list. <br />
          Then enter the position held and years working in that position.
          <br />
          Click "Next" when you're done adding Companies.
        </p>
        <br />
      </div>
      {fields.map((val, idx) => {
